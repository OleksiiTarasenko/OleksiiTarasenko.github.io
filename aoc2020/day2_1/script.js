...
function report(input) {
  ...
  let inputArrayWorking = inputArray.filter(
    (x) => x <= 2020 - inputArray[0] - inputArray[1]
  );

  let sumArray = [];
  for (let i = 0; i < inputArrayWorking.length; i++) {
    for (let j = 0; j < inputArrayWorking.length; j++) {
      i == j
        ? sumArray.push(-1)
        : sumArray.push(inputArrayWorking[i] + inputArrayWorking[j]);
    }
  }

  let arr = sumArray.map((x) => 2020 - x);

  for (let i = 0; i < arr.length; i++) {
    if (inputArrayWorking.indexOf(arr[i]) != -1) {
      return (
        inputArrayWorking[i % inputArrayWorking.length] *
        inputArrayWorking[Math.floor(i / inputArrayWorking.length)] *
        inputArrayWorking[inputArrayWorking.indexOf(arr[i])]
      );
    }
  }
}
